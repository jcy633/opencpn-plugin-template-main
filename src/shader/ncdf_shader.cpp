...
static const char* s_frag =
    ...
    "\n"
    "    if (vectorMode == 1) {\n"
    "        // Vector mode: R=u, G=v. Interpolate both channels, then compute speed.\n"
    "        float uInt, vInt;\n"
    "        if (mode == 0) {\n"
    "            // Bilinear\n"
    ...
    "            float wSum=1.0,uV=center.r,vV=center.g;\n"
    "            if(s10.a>0.01){uV+=f.x*s10.r;vV+=f.x*s10.g;wSum+=f.x;}\n"
    "            if(s01.a>0.01){uV+=f.y*s01.r;vV+=f.y*s01.g;wSum+=f.y;}\n"
    "            if(s11.a>0.01){uV+=f.x*f.y*s11.r;vV+=f.x*f.y*s11.g;wSum+=f.x*f.y;}\n"
    "            uInt=uV/wSum; vInt=vV/wSum;\n"
    "        } else {\n"
    "            // Bicubic or PCHIP: interpolate R and G separately\n"
    "            vec4 wx=cubic(f.x),wy=cubic(f.y);\n"
    "            float uSum=0.0,vSum=0.0,wSum=0.0;\n"
    "            for(int j=-1;j<=2;j++){\n"
    "                for(int i=-1;i<=2;i++){\n"
    "                    vec2 suv=(tc+vec2(float(i),float(j))+0.5)*ts;\n"
    "                    vec4 s=texture2D(dataTex,suv);\n"
    "                    if(s.a>0.01){float w=wx[i+1]*wy[j+1];uSum+=s.r*w;vSum+=s.g*w;wSum+=w;}\n"
    "                }\n"
    "            }\n"
    "            if(wSum<=0.0)discard;\n"
    "            uInt=uSum/wSum; vInt=vSum/wSum;\n"
    "        }\n"
    "        result = sqrt(uInt*uInt + vInt*vInt);\n"
    "        // Normalize to [0, 1] using dataMin/dataMax (speed range)\n"
    "        float range = dataMax - dataMin;\n"
    "        if (range > 0.0001) result = (result - dataMin) / range;\n"
    "        result = clamp(result, 0.0, 1.0);\n"
    "    } else {\n"
    "    // Scalar mode: original code\n"
    "        vec4 s11=texture2D(dataTex,vUV+vec2(ts.x,ts.y));\n"
    ...
    "            float sv[16];float gv[16];bool hv[16];\n"
    "            for(int j=-1;j<=2;j++){\n"
    "                for(int i=-1;i<=2;i++){\n"
    "                    int idx=(j+1)*4+(i+1);\n"
    "                    vec2 suv=(tc+vec2(float(i),float(j))+0.5)*ts;\n"
    "                    vec4 s=texture2D(dataTex,suv);\n"
    "                    sv[idx]=s.r;gv[idx]=s.g;hv[idx]=(s.a>0.01);\n"
    "                }\n"
    "            }\n"
    ...
    "                rowG[j]=pchipRow(gv[j*4],hv[j*4],gv[j*4+1],hv[j*4+1],\n"
    "                    gv[j*4+2],hv[j*4+2],gv[j*4+3],hv[j*4+3],f.x,rowSl[j]);\n"
    "                rowHas[j]=hv[j*4+1]||hv[j*4+2];\n"
    "            }\n"
    "            float val=0.0,gVal=0.0,wSum=0.0;\n"
    "            for(int j=0;j<4;j++){\n"
    ...
    "                val+=rowVal[j]*w;gVal+=rowG[j]*w;wSum+=w;\n"
    "            }\n"
    "            if(wSum<=0.0)discard;\n"
    "            if(vectorMode==1){\n"
    "                float uI=val/wSum, vI=gVal/wSum;\n"
    "                result=sqrt(uI*uI+vI*vI);\n"
    "                float range=dataMax-dataMin;\n"
    "                if(range>0.0001) result=(result-dataMin)/range;\n"
    "                result=clamp(result,0.0,1.0);\n"
    "            } else {\n"

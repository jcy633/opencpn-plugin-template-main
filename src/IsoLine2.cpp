/**********************************************************************
zyncdf: meteorological ncdf file viewer
Copyright (C) 2008 - Jacques Zaninetti - http://www.zyncdf.org
...
//---------------------------------------------------------------
IsoLine::IsoLine(double val,  double **rec_, wxUint32 latSectors, wxUint32 lonSectors, 
		 wxDouble tlat, wxDouble tlon, wxDouble incrLat, wxDouble incrLon)
{
    value = val;
    rec = rec_;
    W = lonSectors; //rec->getNi();
    H = latSectors; //rec->getNj();
    this->incrLat = incrLat;
    this->incrLon = incrLon;
    this->tlat = tlat;
    this->tlon = tlon;
...
void IsoLine::drawIsoLineLabels(wxDC *pmdc, wxColour couleur,
                                PlugIn_ViewPort *vp,
...
Segment::Segment(int I, int J,
                ...
                double **rec, double pressure,wxDouble tlat,wxDouble tlon, wxDouble incrLat, wxDouble incrLon)
{
    this->tlat = tlat; this->tlon = tlon;
    this->incrLat = incrLat; this->incrLon = incrLon;
...
void Segment::intersectionAreteGrille(int i,int j, int k,int l, double *x, double *y,
               double **rec, double pressure)
{
    double a,b, pa, pb, dec;
    pa = rec[j][i];//rec->getValue(i,j);
    pb = rec[l][k];//rec->getValue(k,l);
    // Abscisse
 //           inline double  getX(int i) const   { return ok ? Lo1+i*Di : ncdf_NOTDEF;}
 ...
 //   pj = (py-message->firstGridPointLat)/message->jDirectionIncr;//(py-La1)/Dj;            
    a = tlon+i*incrLon;//rec->getX(i);
    b = tlon+k*incrLon;//rec->getX(k);
    if (pb != pa)
...
//-----------------------------------------------------------------------
// Génère la liste des segments.
// Les coordonnées sont les indices dans la grille du ncdfRecord
//---------------------------------------------------------
void IsoLine::extractIsoLine(double **rec)
{
    int i, j;
    double a,b,c,d;
 //   W = lonSectors;//rec->getNi();
        ...
        for (i=1; i<W; i++)
        {
          ...
          //  y = rec->getY(j);
            ...
            if     ((a<=value && b<=value && c<=value  && d>value)
                ...
                trace.push_back(new Segment(i,j, 'c','d',  'b','d', rec,value,tlat,tlon,incrLat,incrLon));
            else if ((a<=value && c<=value && d<=value  && b>value)
                 || (a>value && c>value && d>value  && b<=value))
                trace.push_back(new Segment(i,j, 'a','b',  'b','d', rec,value,tlat,tlon,incrLat,incrLon));
            else if ((c<=value && d<=value && b<=value  && a>value)
                 || (c>value && d>value && b>value  && a<=value))
                trace.push_back(new Segment(i,j, 'a','b',  'a','c', rec,value,tlat,tlon,incrLat,incrLon));
            else if ((a<=value && b<=value && d<=value  && c>value)
                 || (a>value && b>value && d>value  && c<=value))
                trace.push_back(new Segment(i,j, 'a','c',  'c','d', rec,value,tlat,tlon,incrLat,incrLon));
            //--------------------------------
            // 1 segment H ou V
            //--------------------------------
            else if ((a<=value && b<=value   &&  c>value && d>value)
                 || (a>value && b>value   &&  c<=value && d<=value))
                trace.push_back(new Segment(i,j, 'a','c',  'b','d', rec,value,tlat,tlon,incrLat,incrLon));
            else if ((a<=value && c<=value   &&  b>value && d>value)
                 || (a>value && c>value   &&  b<=value && d<=value))
                trace.push_back(new Segment(i,j, 'a','b',  'c','d', rec,value,tlat,tlon,incrLat,incrLon));
            //--------------------------------
            ...
            //--------------------------------
                ...
                trace.push_back(new Segment(i,j, 'a','b',  'a','c', rec,value,tlat,tlon,incrLat,incrLon));
                trace.push_back(new Segment(i,j, 'b','d',  'c','d', rec,value,tlat,tlon,incrLat,incrLon));
            }

                                           ...
                                           int dataDim[2], int glDim[2])
{
    ...
    if (!m_pdc) {
        ...
        if (hasTex && texID) {
            ...
            // === Shader path (optional, auto-fallback) ===
            if (m_useShader && ncdf_shader_initialized()) {
                GLuint prog = ncdf_shader_get_grid_program();
                NcdfShaderUniforms u = ncdf_shader_get_uniforms();

                ncdf_shader_use_program(prog);
                ...
                if (u.enableAA >= 0) ncdf_shader_uniform_1i(u.enableAA, 1);
                if (u.dataTex >= 0) {
                    ncdf_shader_active_texture(0x84C0); // GL_TEXTURE0
                    glBindTexture(GL_TEXTURE_2D, texID);
